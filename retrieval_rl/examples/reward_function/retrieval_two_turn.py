...
def format_reward(response: str) -> float:
    ...
    if matches_in_rewrite != []:
        score+=0.5
    if len(matches_in_rewrite_t2) >=3 :
        score+=0.5
...
def retrieval(model_address, queries, corpus, score_function="cos_sim", top_k=10, retrieval_batch_size = 1000):
    """
...
def compute_score(reward_inputs: list[dict[str, Any]], model_address, corpus, score_function, retrieval_lookup_table, top_k=10, retrieval_batch_size = 1000, format_weight: float = 0.1) -> list[dict[str, float]]:
    ...
    for idx, reward_input in enumerate(reward_inputs):
        ...
        if format_score == 1:
            ...
            matches_in_rewrite_t2 = re.findall(r"<rewrite_2>([^<]*)</rewrite_2>", response, re.DOTALL)
            matches_t2 = matches_in_rewrite_t2[2:]
            query2rewrite_t2[idx] = [x+len(rewrite_queries_t2) for x in range(len(matches_t2))]
            rewrite_queries_t2 = rewrite_queries_t2 + matches_t2

            #print('****',matches, matches_t2)
    ...
    retrieval_result_t2 = retrieval(model_address, rewrite_queries_t2, corpus, score_function, top_k, retrieval_batch_size)
    for idx, reward_input in enumerate(reward_inputs):
        if scores[idx]["format"] != 1:
            scores[idx]["accuracy"] = 0
            scores[idx]["acc_t1"] = 0
            scores[idx]["acc_t2"] = 0
            scores[idx]["overall"] = format_weight * scores[idx]["format"]
        else:
            ...
            rewrite_ids_t2 = query2rewrite_t2[idx]
            one_query_retrieval_result_t2 = []
            for x in rewrite_ids_t2:
                one_query_retrieval_result_t2 = one_query_retrieval_result_t2 + retrieval_result_t2[x]
            one_query_retrieval_result_t2 = merge_retrieval_result(one_query_retrieval_result_t2)
            one_query_retrieval_result_t2 = one_query_retrieval_result_t2[:top_k]
            #print(one_query_retrieval_result, [retrieval_lookup_table[x] for x in reward_input["ground_truth"]])
            one_query_retrieval_result_t2 = [x[1] for x in one_query_retrieval_result_t2]
            accuracy_score_t2 = accuracy_reward(one_query_retrieval_result_t2, reward_input["ground_truth"], retrieval_lookup_table)

            scores[idx]["acc_t1"] = accuracy_score
            scores[idx]["acc_t2"] = accuracy_score_t2
            scores[idx]["accuracy"] = accuracy_score*0.5 + accuracy_score_t2
            scores[idx]["overall"] = (1 - format_weight) * (accuracy_score*0.5 + accuracy_score_t2) + format_weight * scores[idx]["format"]
