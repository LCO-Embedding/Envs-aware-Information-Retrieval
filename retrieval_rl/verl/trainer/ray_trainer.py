...
class RayPPOTrainer:
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            if self.config.worker.rollout.rollout_mode == 'one_turn':
                test_output_gen_batch = self.actor_rollout_ref_wg.generate_sequences(test_gen_batch)
            elif self.config.worker.rollout.rollout_mode == 'two_turn':
                test_output_gen_batch = self.actor_rollout_ref_wg.generate_sequences_twoturn(test_gen_batch)
            elif self.config.worker.rollout.rollout_mode == 'two_turn_balanced':
                test_output_gen_batch = self.actor_rollout_ref_wg.generate_sequences_twoturn_balanced(test_gen_batch)
            else:
                print(f'Do not find rollout mode {self.config.worker.rollout.rollout_mode} in rollout!')
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            if self.config.worker.rollout.rollout_mode == 'one_turn':
                ...
                gen_batch_output = self.actor_rollout_ref_wg.generate_sequences_twoturn(gen_batch)
            elif self.config.worker.rollout.rollout_mode == 'two_turn_balanced':
                gen_batch_output = self.actor_rollout_ref_wg.generate_sequences_twoturn_balanced(gen_batch)
            else:
                print(f'Do not find rollout mode {self.config.worker.rollout.rollout_mode} in rollout!')
            ...
            if self.config.algorithm.adv_estimator == "remax":
                ...
                if self.config.worker.rollout.rollout_mode == 'one_turn':
                    ...
                    gen_baseline_output = self.actor_rollout_ref_wg.generate_sequences_twoturn(gen_baseline_batch)
                elif self.config.worker.rollout.rollout_mode == 'two_turn_balanced':
                    gen_baseline_output = self.actor_rollout_ref_wg.generate_sequences_twoturn_balanced(gen_baseline_batch)
                else:
                    print(f'Do not find rollout mode {self.config.worker.rollout.rollout_mode} in rollout!')
