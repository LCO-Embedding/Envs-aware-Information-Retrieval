...
class BM25QueryEncoder:
    def __init__(self, metadata_path="bm25_index_RAGBench_metadata.json"):
        # Load metadata
        with open(metadata_path, "r") as f:
            data = json.load(f)
            
        self.vocab = data["vocab"] # {"term": index}
        self.idf = np.array(data["idf_vector"]) # Array of floats
        self.k1 = data["params"]["k1"]
        
    def encode(self, query_text):
        """
        Converts query text -> Sparse Vector Indices and Values.
        Returns a dict compatible with PyTorch sparse tensors.
        """
        # Simple tokenization (must match the regex in the training script: r"(?u)\b\w+\b")
        # If you used a specific tokenizer (like BERT), use that here too.
        import re
        tokens = re.findall(r"(?u)\b\w+\b", query_text.lower())
        ...
        for token in tokens:
            if token in self.vocab:
                ...
                indices.append(idx)
                # Query vector value = IDF of the term
                values.append(self.idf[idx])
        
        # Format for PyTorch sparse construction
        # Return {indices: [1, 5, 20], values: [1.2, 0.5, 2.1]}
...
def call_server(queries, corpus_tensor, encoder):
    query_embeddings = []

    for q in queries:
        q_data = encoder.encode(q)
        cols = q_data['indices']
        rows = [0] * len(cols)

        # Shape: (2, nnz)
        q_indices = torch.tensor([rows, cols], dtype=torch.long)
        q_values = torch.tensor(q_data['values'], dtype=torch.float32)

        vocab_size = corpus_tensor.shape[1]

        # Create Sparse COO, then convert to DENSE for multiplication
        # torch.sparse.mm requires (Sparse) @ (Dense)
        query_vec_sparse = torch.sparse_coo_tensor(
            q_indices, 
            q_values, 
            size=(1, vocab_size)
        )

        # Convert to dense and transpose to make it a column vector (Vocab x 1)
        query_vec_dense = query_vec_sparse.to_dense().t()

        query_embeddings.append(query_vec_dense)

    output_query_embeddings = torch.cat(query_embeddings, dim=1) #(Vocab x n)

    return output_query_embeddings


def retrieval(queries, corpus, bm25_encoder, top_k=10, retrieval_batch_size = 1000):
    query_ids = list(range(len(queries)))

    start_time = time.perf_counter()

    query_embeddings = call_server(queries, corpus, bm25_encoder)

    vacab_size, num_query = query_embeddings.shape
    ...
    corpus_start_idx = 0
    sub_corpus_embeddings = corpus

    # Compute similarites using either cosine-similarity or dot product

    start_time = time.perf_counter()

    # (N_docs, Vocab) @ (Vocab, n) -> (N_docs, n)
    cos_scores = torch.sparse.mm(sub_corpus_embeddings, query_embeddings)
    # (N_docs, n) -> (n, N_docs)
    cos_scores = cos_scores.t()
    ...
    for query_itr in range(num_query):
...
def compute_score(reward_inputs: list[dict[str, Any]], model_address, corpus, score_function, retrieval_lookup_table, top_k=10, retrieval_batch_size = 1000, format_weight: float = 0.1) -> list[dict[str, float]]:
    ...
    #bm25 related
    scipy_matrix = sp.load_npz("/root/a100/chx/a_serve_embedding/bm25_index_RAGBench_corpus.npz")
    
    values = torch.tensor(scipy_matrix.data, dtype=torch.float32)
    col_indices = torch.tensor(scipy_matrix.indices, dtype=torch.long)
    row_ptr = torch.tensor(scipy_matrix.indptr, dtype=torch.long)

    corpus_tensor = torch.sparse_csr_tensor(
        row_ptr, col_indices, values, size=scipy_matrix.shape
    )

    encoder = BM25QueryEncoder(metadata_path='/root/a100/chx/a_serve_embedding/bm25_index_RAGBench_metadata.json')

    #start reward computing
    scores = []
    query2rewrite = {}
    rewrite_queries = []
    ...
    retrieval_result = retrieval(rewrite_queries, corpus_tensor, encoder, top_k, retrieval_batch_size)
    for idx, reward_input in enumerate(reward_inputs):
